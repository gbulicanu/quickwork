...
def process_jira(jira_result, days=-1):
  ...
  for issue_key in issue_keys:
    json_path_issue = parse("$.issues[?(@.key='{}')]".format(issue_key))
    ...
    json_path_worklogs = parse(
      "$.fields.worklog.worklogs[?(@.author.emailAddress='gheorghe@gocontractor.com' & @.started >= '{}')]"
        .format(from_time)
      )
    json_path_time_spends = parse("timeSpentSeconds")
    json_path_comment = parse("comment.content[0].content[0].text")
    worklogs = filter(lambda x: __filter_worklog(x, from_time), [match.value for match in json_path_worklogs.find(issue)])
